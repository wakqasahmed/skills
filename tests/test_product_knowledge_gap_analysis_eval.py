...
class ProductKnowledgeGapAnalysisEvalTest(unittest.TestCase):
    ...
    def test_harness_rejects_forged_labels_without_response_evidence(self):
        ...
        failures, _ = load_module(VALIDATOR, "product_knowledge_forged_label_validator").validate(records)

        self.assertTrue(any("/enabled is below" in failure for failure in failures))

    def test_isolated_workspace_hides_rubric_and_has_no_network(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(json.loads((workspace / "case.json").read_text()), {"id": case["id"], "prompt": case["prompt"]})
