...
class ValidatePluginTest(unittest.TestCase):
    ...
    def test_aggregate_sync_commits_manifest_on_drift(self) -> None:
        ...
        self.assertIn("git add -A skills .claude-plugin/plugin.json README.md", workflow)
    ...
    def make_source_repositories(self, root: Path) -> list[Path]:
        source_roots = [root / name for name in ("visibility", "commerce", "workflow", "laravel", "email")]
        source_skills = [
    ...
    def test_sync_updates_readme_navigation_for_added_source_skill(self) -> None:
