...
5. **Final OCR/CI settling gate** — after the final push, wait for OCR and CI to settle. Run the `ai-agent-pr-metadata` final OCR disposition gate against the current head SHA. Each latest-head OCR finding must be recorded as `fixed` (commit and test), `deferred` (linked issue), or `declined` (concise technical reason). An undispositioned finding blocks merge. Only correctness, security, data integrity, and acceptance-criteria findings are blocking and must be fixed; do not convert non-blocking suggestions into blanket code churn.
