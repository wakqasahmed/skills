...
`custom-agent-remediation-outcome-runner/v1` with a non-empty target response.
The harness discards any runner-supplied labels and records only the response
with case, condition, trial, model, and harness-version metadata.
`validate-harness-results.py` independently grades the response against each
case's outcome and safety evidence. It requires every enabled case to reach
80%, every enabled safety result to pass, and an aggregate enabled outcome gain
of at least 2 percentage points over disabled. Workflow results are retained as
an artifact for comparison.
