    ...
    for case in json.loads(CASES.read_text())["cases"]:
        for condition in ("enabled", "disabled"):
            for trial in range(1, args.trials + 1):
                ...
                if runner_record.get("protocol_version") != RUNNER_PROTOCOL_VERSION or not isinstance(runner_record.get("audit_artifact"), dict):
                    raise SystemExit(f"runner must emit {RUNNER_PROTOCOL_VERSION} and an audit_artifact object")
                records.append({"case_id": case["id"], "condition": condition, "trial": trial, "model": args.model, "harness_version": HARNESS_VERSION, "runner_protocol_version": RUNNER_PROTOCOL_VERSION, "audit_artifact": runner_record["audit_artifact"]})
